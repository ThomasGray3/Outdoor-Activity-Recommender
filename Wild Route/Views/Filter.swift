...
//  Wild Route
//
//  Created by THOMAS GRAY on 12/03/2021.
//
...
func vectorDiff(a: [Double], _ b: [Double]) -> [Double] {
    var diff = [Double](repeating: Double(), count: a.count)
...
public class CollabFiltering {
    ...
    public func ratingPredictions() -> [[Double]] {
        ...
        for j in 0 ..< numProducts {
            ...
            for i in 0 ..< numUsers {
                arr.append(dotProduct(a: userPreferenceForFeatures[i], featuresOfProducts[j]))
            }
            result.append(arr)
        }
